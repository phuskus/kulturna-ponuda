...
public class PostServiceIntegrationTest {
	...
	public void createAndDelete_EntitySavedAndDeleted() {
		...
		PostDTO newPostDTO = new PostDTO(DB_NEW_POST_TITLE, DB_NEW_POST_CONTENT, DB_CULTURAL_OFFER_ID,
				DB_CULTURAL_OFFER_NAME, pictures);
		PostDTO createdPost = postService.create(newPostDTO, null);
	...
	public void updatePost_PostExists_EntityUpdatedAndReveretedToPreviousState() {
		...
		PostDTO dto = new PostDTO(post.getId(), DB_NEW_POST_TITLE, DB_NEW_POST_CONTENT, new Date(),
				post.getCulturalOffer().getId(), post.getCulturalOffer().getName());
		...
		// clean up
		PostDTO oldDTO = new PostDTO(post.getId(), post.getTitle(), post.getContent(), new Date(),
				post.getCulturalOffer().getId(), post.getCulturalOffer().getName());
