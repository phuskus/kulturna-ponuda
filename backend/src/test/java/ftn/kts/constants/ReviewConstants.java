package ftn.kts.constants;

public class ReviewConstants {
    public static String NONEXISTENT_FIELD = "name";
    public static Long EXISTENT_ID = 2L;
    public static Long EXISTENT_USER_ID = 2L;
    public static Long EXISTENT_OFFER_ID = 1L;
    public static Long NONEXISTENT_OFFER_ID = 132132L;
    public static Long NONEXISTENT_ID = 177L;
    public static String EXISTEND_USERNAME = "test2@gmail.com";

    public static String CONTENT = "Ne valja nista";
    public static String REVIEW_CONTENT = "Nice review";
    public static String NEW_CONTENT = "Super";
    public static String RANDOM_STRING = "FLDSKJLFKSDFH KJDLSkf jdhssfkljfdsal;kfd";
    public static Long NEW_RATING = 4L;

    public static final long NUM_ITEMS = 3;
    public static final long NUM_REVIEWS_FOR_OFFER = 3;
    public static final int PAGEABLE_PAGE = 0;
    public static final int PAGEABLE_SIZE = 2;
}
